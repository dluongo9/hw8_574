...
class Seq2SeqModel(nn.Module):
    ...
    def forward(
        ...
        source_padding_mask = self.get_padding_mask(
            ...
            key_lengths=lengths[0]  # =source_lengths
        )
        ...
        # (batch_size, target_sequence_length, vocab_size)
        decoded = self.decode(h_c, embedded_target, lengths[1], encodings, source_padding_mask)[0]
        return decoded
    ...
    def encode(
        ...
        packed = nn.utils.rnn.pack_padded_sequence(source_embeddings, source_lengths, batch_first=True,
                                                   enforce_sorted=False)
                
        # Get packed output and (h,c) from encoder
        # Hint: be sure of the return signature for torch.nn.LSTM
        output, h_c = self.encoder(packed)
                
        # Unpack the encoder output
        # (batch_size, source_sequence_length, hidden_dim)
        unpacked = nn.utils.rnn.pad_packed_sequence(output, batch_first=True)

        # Return the encoder output and tuple of hidden and cell statess
        return unpacked[0], (h_c[0], h_c[1])
    ...
    def decode(
        ...
        packed = nn.utils.rnn.pack_padded_sequence(input=target_embeddings, lengths=target_lengths, batch_first=True,
                                                   enforce_sorted=False)
        # Get packed output and (h,c) from the decoder
        output, h_c = self.decoder(packed, inits)
    ...
    def attention(
        ...
        compatability = torch.matmul(decoder_states, torch.transpose(encoder_states, 1, 2))#torch.transpose(encoder_states, dim0=1, dim1=0))
        if padding_mask is not None:
            compatability += padding_mask
        #torch.soft
        attention = torch.nn.functional.softmax(compatability, dim=2)
        return torch.matmul(attention, encoder_states)
